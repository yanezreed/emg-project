...
Having gained confidence working with both C code and Python throughout my studies in Harvard's CS50 and CS50P courses. Because of this, I have now made the decision to develop my application primarily with Python.
...
​
Python's library ecosystem was also a major factor. Supporting the core functionality for my project with OAuth authentication, API communication, and the integration of my own AI models, all acting as vital components for my application. Working with these technologies at a higher level, rather than at a lower level in C code, is definitely more manageable for me at this stage. Not having to worry about manual memory management, pointer mismanagement, buffer overflow or any kind of low level error easily created in C, allows me to focus on understanding the underlying concepts before anything else.

Which will perfectly fit my incremental approach. As I will be developing each piece of my application separately, to ultimately integrate the various core components later on.
...
Having previously handled memory management, network requests, and parsing json structures within my previous project, in a C code HTTP server. I’m well aware that the easily made errors in any of these areas could cause critical faults or security vulnerabilities within both development, and within the business when deployed. The additional time it would take, in low level implementation and testing to ensure any errors are dealt with, would be substantial and unrealistic within my timescope.
​
